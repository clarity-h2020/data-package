...
#RAILWAYS (12230)
ogr2ogr -sql "SELECT area,perimeter,CODE2012 FROM "$SHP" WHERE CODE2012='12230'" $NAME $FILE
...
#psql -U "postgres" -d "clarity" -c "INSERT INTO railways(SELECT NEXTVAL('railways_gid_seq'), area, perimeter, code2012, geom, albedo, emissivity, transmissivity, vegetation_shadow, run_off_coedfficient, building_shadow FROM "$NAME");"
